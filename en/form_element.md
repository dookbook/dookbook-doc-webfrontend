...
| `pattern` | The `pattern` attribute specifies the regular expression used to validate the value of the `<input>` element。Applies to `type`: *`text`*, *`search`*, *`url`*, *`tel`*, *`email`* 和 *`password`*。|
