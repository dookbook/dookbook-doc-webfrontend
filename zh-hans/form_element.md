...
| **`value`** | 输入的**当前值**。|
| **`required`** | *布尔*值，指示输入字段的值是**必需的**。如果指定该属性，则元素可应用 **`:required`** 伪类；否则可应用 **`:optional`** 伪类。不可用于 `type` 为 *`submit`*，*`color`*，*`hidden`*，*`range`*，*`image`*，*`reset`*，*`button`* 的输入框。<br>同时如果设置了该属性，当鼠标悬挑时会出现 “请填写此字段。”的字样。可用 `text`改变默认字样。|
